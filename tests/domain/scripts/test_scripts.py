...
from .. import grapa_folder, Graph, GrapaWarning, HiddenPrints
...
@pytest.fixture
def script_correlations_setup_A(grapa_folder, script_correlations_setup_base):
...
@pytest.fixture
def script_correlations_setup_B(grapa_folder, script_correlations_setup_base):
    filename = os.path.join(
        grapa_folder,
        "examples",
        "JV",
...
def test_script_correlations_A1(grapa_folder, script_correlations_setup_A):
    from grapa.scripts.script_correlations import CONF, process_file

    fname, other = script_correlations_setup_A
    dkeys, filt, xs, ys = other
    # dkeys = AS_DATATABLE  # not necessarily needed, autodetection should work
    filt = [["Jsc_mApcm2", CONF.HIGHER, 32.1]]
    # dkeys = ["Voc", "Jsc", "FF", "area"]  # , 'Eff', 'Rp', 'acquis soft rs']
    xs = range(1, 11)
    ys = range(1, 5)
    with pytest.warns(ConstantInputWarning, match=r"An input array is constant"):
        # warning because area is constant
        with HiddenPrints():
            out = process_file(
                fname, datakeys=dkeys, filters=filt, idx_pattern=xs, idx_labels=ys
            )
    script_boxplot_assert(out)
    assert len(out[1].y()) == 11  # 11 items after filtering Jsc > 32.1
    assert out[1].y()[:3] == pytest.approx([0.714, 0.715, 0.714])
    assert out[3].x()[:3] == pytest.approx([32.2, 32.4, 32.3])
    assert len(out) == 2 * len(xs) * len(ys)
...
@pytest.fixture
def script_jv_setup_b(grapa_folder):
    folder = os.path.join(grapa_folder, "examples", "JV", "SAMPLE_B_3layerMo")
    yield folder
    # after yield: teardown code
    # cleanup files generated by the script
...
def test_script_correlations_jvall(script_jv_setup_b):
    from grapa.scripts.script_correlations import CONF, process_file
    from grapa.scripts.script_processJV import processJVfolder

    folder = script_jv_setup_b
    with HiddenPrints():
        processJVfolder(folder, groupCell=True, fitDiodeWeight=5, pltClose=True)

    summary = os.path.join(folder, "export_SAMPLE_B_3LayerMo_summary_allJV.txt")
    assert os.path.exists(summary)

    datakeys = ["Voc", "Jsc", "FF", "Eff", "area"]
    with HiddenPrints():
        out = process_file(
            summary,
            datakeys=datakeys,
    ...
    script_boxplot_assert(out)
    assert len(out) == 2 * len(datakeys) * len(datakeys)
    assert len(out[1].x()) == 18
    assert out[1].y()[:3] == pytest.approx(
        [0.7214836486176838, 0.7168525559768821, 0.716713019557396]
    )
    assert out[3].x()[:3] == pytest.approx(
        [33.56100622836693, 33.59125954800677, 34.13326177216637]
    )
    assert os.path.exists(
        os.path.join(
            folder, "export_SAMPLE_B_3LayerMo_summary_allJV_correlation_data.png"
        )
    )
    assert os.path.exists(
        os.path.join(
            folder, "export_SAMPLE_B_3LayerMo_summary_allJV_correlation_pearson.png"
        )
    )
...
def test_script_jv_mix(script_jv_setup_mix):
    from grapa.scripts.script_processJV import processJVfolder
