...
from grapa.shared.logging_utils import setup_logging
...
from grapa.frontend.observable import Observable
from grapa.frontend.frame_curveactions import GUIFrameActionsCurves
from grapa.frontend.widgets_graphmanager import GraphsTabManager, RecorderSpecialKeys
from grapa.frontend.frames_main import (
    GUIFrameMenuMain,
    GUIFrameConsole,
    GUIFrameCentral,
...
def _stdout_redirect(where, logger_handler: Optional[logging.StreamHandler]):
    ...
    if logger_handler is not None and hasattr(logger_handler, "setStream"):
        # added in python 3.7
...
class Application(tk.Frame):
    ...
    def __init__(
        ...
        if not config_file.startswith("--rootdir="):  # happens with e.g. pytest
            self.newgraph_kwargs["config"] = config_file
        self.figure_factory = MplFigureFactory(use_pyplot=False)
        ...
        if open_file is None:
            filename = "../examples/subplots_examples.txt"
            open_file = os.path.normpath(
                os.path.join(os.path.dirname(os.path.abspath(__file__)), filename)
            )
        self.open_file(open_file)
        self.update_ui()
        # log_special to avoid prompt when close default tab
    ...
    def merge_graph(self, graph):
        ...
        if self.if_print_commands():
            print(f"graph.merge(Graph('{file}'))")
        self.update_ui()
    ...
    def save_graph(self, filesave, fileext="", save_altered=False, if_compact=True):
        ...
        if graph.attr("meastype") == "":
            graph.update({"meastype": GRAPHTYPE_GRAPH})
...
def build_ui(open_file: Optional[str] = None, config_file: Optional[str] = None):
    """To start the main grapa app"""
    logger_handler = setup_logging()
    ...
    if config_file is None and len(sys.argv) > 1:
        ...
        # msg = "Config filename retrieved from sys.argv ({})"
        # print(msg.format(config_file))

    # starts running programm
    app = Application(master=root, open_file=open_file, config_file=config_file)
    with _stdout_redirect(app.frames["console"].get_console(), logger_handler):
