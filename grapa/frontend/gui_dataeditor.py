...
from grapa.frontend.widgets_custom import EntryVar, LabelVar, ComboboxVar
...
class GuiDataEditor(tk.Frame):
    ...
    def __init__(self, master, graph: "Graph", callback, curve_i=0):
    ...
    def save_data(self):
        ...
        x_[i] = x
        y_[i] = y
        self.graph[c].set_x(x_)
        self.graph[c].set_y(y_)
    ...
    def delete_point(self):
        ...
        i = int(self._headCurve3.get())
        curve = self.graph[c]
        data = np.delete(curve.get_data(), i, axis=1)
        curve.set_data(data)
        self._fill_ui_curve_data(c)
