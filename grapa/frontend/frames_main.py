...
from grapa.colorscale import Colorscale, colorscales_from_config

from grapa.shared.conditional_property_applier import ConditionalPropertyApplier
from grapa.shared.constants_internals import GRAPHTYPE_GRAPH
from grapa.shared.error_management import (
    GrapaWarning,
    GrapaError,
    issue_warning,
    IncorrectInputError,
...
from grapa.shared.string_manipulations import strToVar, varToStr, listToString
from grapa.shared.varia import get_point_closest_to_xy
from grapa.shared.keywords_loader import keywords_graph, keywords_curve

from grapa.frontend.gui_dataeditor import GuiDataEditor
from grapa.frontend.gui_texts import GuiManagerAnnotations
from grapa.frontend.image_to_clipboard import image_to_clipboard
from grapa.frontend.interface_openbis import GrapaOpenbis
from grapa.frontend.widgets_graphmanager import (
    GraphsTabManager,
    userconfirm_close_notsaved,
)
from grapa.frontend.widgets_tooltip import CreateToolTip
from grapa.frontend.widgets_custom import (
    bind_tree,
    TextWriteable,
...
class GUIFrameCanvasGraph:
    ...
    def update_ui(self):
        ...
        try:
            with warnings.catch_warnings():
                ...
                fig, ax = Graph.plot(
                    self.app.graph(),
                    fig_ax=[self.fig, None],
                    figure_factory=self.app.figure_factory,
                )
    ...
    def _cw_canvas(self, frame):
        ...
        self.fig = self.app.figure_factory.figure(figsize=figsize, dpi=dpi)
    ...
    def save_point(self):
        ...
        if len(c) == 0:
            ...
            if curve.has_attr("Curve"):
                casted = curve.cast_as(curve.attr("Curve"))
...
class GUIFrameMenuMain:
    ...
    def save_graph(self):
        ...
        if self.app.graph().attr("meastype") not in ["", GRAPHTYPE_GRAPH]:
            msg = (
    ...
    def script_jv_sample_maps(self):
        ...
        if file is not None and file != "":
            ...
            filelist, fileout = processSampleCellsMap(
                file, newGraphKwargs=ngkw, figure_factory=self.app.figure_factory
            )
            if len(fileout) > 0:
    ...
    def script_cf(self):
        ...
        if folder is not None and folder != "":
            ngkw = self.app.newgraph_kwargs
            graph = script_processCf(
                folder, newGraphKwargs=ngkw, figure_factory=self.app.figure_factory
            )
            self.app.open_file(graph)
...
class GUIFrameTemplateColorize:
    ...
    def _cw_content_colors(self, frame):
        ...
        self._widgets["colChoice"].bind("<Return>", lambda event: self.colorize_graph())
        # line 3
        fr = tk.Frame(frmain)
        fr.pack(side="top", anchor="w")
    ...
    def colorize_graph(self):
        ...
        try:
            ...
            # other errors to be printed in GUI console, not hidden in terminal
        ...
        if self.app.if_print_commands():  # to explicit creation of colorscale
            ...
            arg = ", ".join([f"{key}={val!r}" for key, val in kwargs.items()])
...
class GUIFrameActionsGeneric:
    ...
    def _cw_cast(self, frame):
        ...
        def update_var_cast(curve, _key):
            ...
            if isinstance(curve, Curve):
                cast_list = curve.cast_listpossibletypes(only_different=False)
        ...
        for c in curves:
            if -1 < c < len(graph):
                ...
                if self.app.if_print_commands():
                    print(f"graph[{c}].visible(not graph[{c}].visible())")
        self.app.update_ui()
    ...
    @staticmethod
    def _get_curve_data(graph: Graph, c: int, if_altered=True):
        """Returns the x and y data of Curve at index idx, as a 2D numpy array."""
        if if_altered:
            alter = graph.get_alter()
            x = graph[c].x_offsets(alter=alter[0])
            y = graph[c].y_offsets(alter=alter[1])
        else:
            x = graph[c].x(alter="")
            y = graph[c].y(alter="")
        return np.array([x, y])
    ...
    def copy_curve_to_clipboard(self):
        ...
        data = [self._get_curve_data(graph, c, if_altered=if_trans) for c in curves]
    ...
    def cast_curve(self):
        ...
        for curve in curves:
            if -1 < curve < len(graph):
                test = self.app.call_graph_method("curve_cast", type_new, curve)
                selected.append(curve)
                if not test:
                    logger.error("curve_cast impossible.")
            else:
                logger.error("curve_cast impossible (%s, %s)", type_new, curve)
        if len(selected) > 0:
            sel = [graph[c] for c in selected]
            keys = [""] * len(sel)
            self.app.get_tab_properties(
