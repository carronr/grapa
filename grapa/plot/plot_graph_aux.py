...
class ParserAxhline(ParserAxlines):
    ...
    def plot(self, ax, curvedummy: "Curve", alter, type_plot):
        for el in self._list:
            pos = curvedummy.y(
                alter=alter[1],
                xyValue=[1, el["args"][0]],
                error_if_mix_xy=True,
            )
...
class ParserMisc:
    ...
    def set_axislabel(cls, method, label, graph):
        ...
        if isinstance(label, list) and len(label) == 2 and isinstance(label[-1], dict):
            ...
            lbl = graph.format_axis_label(label[0:3])
    ...
    @classmethod
    def alter_lim(cls, ax, lim, xory, alter, curvedummy: "Curve"):
        ...
        liminput = [float(li) if is_number(li) else np.inf for li in lim]
        lim = list(liminput)
        while len(lim) < 2:
            lim.append(np.inf)
        if xory == "x":
