...
from grapa.shared.command_recorder import SuspendCommandRecorder
from grapa.shared.mpl_figure_factory import MplFigureFactory
from grapa.shared.string_manipulations import strToVar, TextHandler
...
from grapa.plot.plot_curve import plot_curve
from grapa.plot.plot_graph_aux import (
    ParserAxhline,
    ParserAxvline,
    ParserMisc,
    GroupedPlotters,
...
def plot_graph(
    ...
    :param img_format: by default image will be .png. Possible format are the ones
           accepted by matplotlib Figure.savefig
    :param figsize: default figure size is a class consant
    :param if_save: if True, save the plot as an image.
    :param if_export: if True, create a human- and machine-readable .txt file
    ...
    :param figure_factory: By default, creates pyplot figures.
           May inject e.g. MplFigureFactory(use_pyplot=False)
    """
    ...
    to_unpack = _create_subplots_gridspec(
        graph, fig_ax, if_subplot=if_subplot, figure_factory=figure_factory
    )
    ...
    to_unpack2 = _plot_loop_over_curves(
        graph, container, ax, handles, figure_factory=figure_factory
    )
...
def _create_subplots_gridspec(
    ...
    if fig_ax is not None:
        ...
        fig = figure_factory.figure()  # create a new figure
        ax = None
    ...
    for curve_i, curve in enumerate(graph):
        ...
        if isinstance(curve, Curve_Inset):
            ...
            try:
                ...
                issue_warning(logger, "%s during inset %s: %s.", type(e), e, val)
                inset = Graph(**graph_aux_kw)
            coords = (
        ...
        if isinstance(curve, Curve_Subplot):
            ...
            if val not in [" ", "", None]:
                try:
                    ...
                    issue_warning(logger, "%s during subplot %s: %s.", type(e), e, val)
...
def _plot_loop_over_curves_colorbar(
    ...
    if isinstance(adjust[-1], str):
        if adjust[-1] == "ax":
            ...
            msg = (
                "plot colorbar: cannot interpret last value of adjust, must be "
                "numeric, or 'ax' (%s)."
            )
...
def _plot_savefig(graph: Graph, fig, filename, img_format):
    ...
    dpi_default = 300
    from_config = graph.config("save_image_dpi", dpi_default)
    save_dpi = graph.attr("dpi", from_config)
