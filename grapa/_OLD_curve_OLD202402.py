...
class Curve:
    ...
    def funcListGUI_boxplot(self, graph, c):
        ...
        at = ["seaborn_stripplot_kw", "seaborn_swarmplot_kw"]
        default = {"color": self.attr("color")}
        if default["color"] == "":
            del default["color"]
        out.append([self.updateValuesDictkeys, 'Save', at,
                    [self.attr(a) for a in at],
                    {'keys': at},
                    [{'field': 'Combobox', 'values': ['', "{}", default], "width": 12},
                     {'field': 'Combobox', 'values': ['', "{}", default], "width": 12}]])
        return out
    ...
    def plot(self, ax, graph=None, graph_i=None, type_plot='', ignoreNext=0, boxplot=None, violinplot=None, violinplotkwargs={}):
        ...
        # a more peculiar plotting
            ...
            if len(y) > 0 and not np.isnan(y).all():
                ...
                # caution version - until 2024.02
                # for key in ['widths', 'notch', 'vert']:
                #     if self.attr(key, None) is not None:
                #         boxplot.update({key: self.attr(key)})
                # brave version - since 2024.02
                for key in fmt:
                    if key not in boxplot and key not in ["label"]:  # this excludes color, color because above
                        boxplot.update({key: fmt[key]})
                self.plot_seaborn_stripswamplots(boxplot['positions'][-1], y, ax)
                boxplot['i'] += 1
    ...
    def plot_seaborn_stripswamplots(self, position, y, ax):
        ...
        if kwstrip is not None:
            try:
                ...
                print("Curve.plot ImportError: could not import library seaborn.")
        kwswarm = self.attr("seaborn_swarmplot_kw", None)
        if kwswarm is not None:
            try:
                import seaborn as sns
                kwswarm = dict(kwswarm)
                kwswarm.update({"ax": ax, "native_scale": True})
                sns.swarmplot({position: y[~np.isnan(y)]}, **kwswarm)
            except ImportError:
                print("Curve.plot ImportError: could not import library seaborn.")
