...
class CurveEQE(Curve):
    ...
    def __init__(self, data, attributes, silent=False):
        ...
        if isinstance(popt, str) and popt == "":
            ...
            try:
                ...
                if not self.silent:
                    print(
                        "ERROR readDataFromFileEQE during Eg calculation",
                        self.filename,
                        self.data[-1].data(),
                    )
    ...
    def funcListGUI(self, **kwargs):
        ...
        if not self.attributeEqual("_popt") and self.attributeEqual(
            ...
            out.append(
                [
                    self.CurveEQE_bandgapDeriv_print,
                    "Bandgap derivative",
        ...
        if not self.attributeEqual("_popt") and self.attributeEqual(
            ...
            out.append(
                [
                    self.CurveEQE_bandgapLog_print,
        ...
        if self.attributeEqual("_popt"):
            ...
            line.append(
                "          spectrum",
                fnames[0],
                options={"width": 15, "field": "Combobox", "values": fnames},
            )
            line.append(
                "integrated current as curve", False, options={"field": "Checkbutton"}
            )
            out.append(line)
        ...
        if self.attributeEqual("_popt"):
            try:  # Urbach fitting
                ...
                print(
                    "WARNING CurveEQE: do not find", "grapa.datatypes.curveArrhenius."
                )
                print(type(e), e)
    ...
    def bandgapFromTauc(self, nm, EQE, yLim=[0.25, 0.70], xLim=[600, 1500], mode="EQE"):
        """
        ...
        if max(EQE) > 10:
            print(
                "Function bandgapFromTauc: max(EQE) > 1. Multiplied by",
    ...
    def CurveEQE_bandgap_print(self, yLim=None):
        ...
        print(
            "Bandgap (Tauc (E*EQE)^2):", roundSignificant(out.attr("_popt")[0], 4), "eV"
        )
        return out
    ...
    def CurveEQE_bandgapLog_print(self, yLim=None):
        ...
        print(
            "Bandgap (Tauc (E*ln(1-EQE))^2):",
            roundSignificant(out.attr("_popt")[0], 4),
            "eV",
        )
        return out
    ...
    def CurveEQE_bandgapLog(self, yLim=None):
        ...
        try:
            yLim = np.array(yLim) / self.attr("mulOffset", default=1)
            bandgap = self.bandgapFromTauc(
                self.x(), self.y(), yLim=yLim, mode="log1-EQE"
            )
        except Exception:
            pass
        x = self.x()
    ...
    @staticmethod
    def func_gaussian_a(x, a, x0, sigma):
        """
    ...
    def bandgapDeriv(self, SGwidth=None, SGdegree=None, x_y_savgol=None, ROI=None):
        ...
        while True:
            ...
            if savgol[i] < testv[1]:
                break
            roifit[1] += 1
        # roifit = [max(0, roifit[0]), min(len(savgol), roifit[1])]
        roifitr = range(*roifit)
        datax, datay = x_[roifitr], savgol[roifitr]
        p0 = [savgol[maxi], x_[maxi], 2 * (datax[1] - datax[0])]

        try:
            popt, pcov = curve_fit(self.func_gaussian_a, datax, datay, p0=p0)
        except Exception as e:
            msg = "Exception CurveEQE.bandgapDeriv. Provided to curve_fit length x {}, length y {}, p0 {}."
    ...
    def bandgapDeriv_savgol(self, SGwidth=None, SGdegree=None, ROI=None):
        ...
        if SGdegree >= SGwidth:
            print(
                "Curve EQE bandgap derivative: degree must be lower than",
                "width for Savitsky-Golay filtering.",
            )
            return None, None, None
        if SGwidth % 2 == 0:
            print(
                "Curve EQE bandgap derivative: width must be odd for",
    ...
    def CurveEQE_bandgapDeriv_print(self, SGwidth=None, SGdegree=None, ROI=None):
        ...
        print(
            "Bandgap (derivative):",
            roundSignificant(out[0].attr("bandgapDeriv"), 4),
            "eV",
        )
        return out
    ...
    def CurveEQE_bandgapDeriv(self, SGwidth=None, SGdegree=None, ROI=None):
        ...
        x_, y_, savgol = self.bandgapDeriv_savgol(
            SGwidth=SGwidth, SGdegree=SGdegree, ROI=ROI
        )
        popt, xr = self.bandgapDeriv(
            SGwidth=SGwidth, SGdegree=SGdegree, x_y_savgol=[x_, y_, savgol]
        )
        bandgap = Curve.NMTOEV / popt[1]
        # label = "{} {} eV".format(self.attr("label"), roundSignificant(bandgap, 4))
        attr = {
            "color": "k",
            "bandgapDeriv": bandgap,
            "data filename": self.attr("filename"),
            "muloffset": self.attr("mulOffset", 1) * 0.1,
            "_bandgapDeriv_savgol": [SGwidth, SGdegree],
            # "label": label,
        }
        out = []
        ...
        fity = self.func_gaussian_a(fitx, *popt)
    ...
    def CurveEQE_returnAM15referenceCurve(self):
        ...
        if CurveEQE.EQE_AM15_REFERENCE is None:
            ...
            if len(refs) <= 1:
                print(
                    "ERROR CurveEQE_returnAM15referenceCurve: cannot find",
                    "reference file",
                    self.EQE_AM15_REFERENCE_FILE,
                )
                return 0
    ...
    def CurveEQE_returnAM0referenceCurve(self):
        ...
        if CurveEQE.EQE_AM0_REFERENCE is None:
            ...
            if refs.length() == 0:
                print(
                    "ERROR CurveEQE_returnAM0referenceCurve: cannot find",
                    "reference file",
                    self.EQE_AM0_REFERENCE_FILE,
                )
                return 0
    ...
    def ERE(self, Voc, T, Emin="auto", EminUnit="nm"):
        ...
        """
        # HERE SUBSTITUTION -> error with prefactor?!
        phi_bb = 1 / (4**2 * CST_hbar**3 * CST_c**2) * E**2 / (np.exp(E / (CST_kb * T)) - 1)
        # Unit: J-3 s-3 m-2 s2 J2 (1) = J-1 s-1 m-2
        integrand = EQE * phi_bb
        j0_rad = np.abs(integrate.trapz(EQE[mask] * phi_bb[mask], x=E[mask]))
        # Unit: J-1 s-1 m-2 J = s-1 m-2
        j_g = Jsc / CST_q  # [s-1 m-2]  A m-2 C-1
        QFLS_rad = CST_kb * T * np.log(j_g / j0_rad)  # [J]

        print("Voc / CST_kb / T", Voc, CST_kb, T, Voc / CST_kb / T)
        ERE = np.exp(CST_q * Voc / CST_kb / T) * j0_rad / j_g
            # QFLS = CST_kb * T * np.log(PLQY * j_g / j0_rad)  # [J]
        lbl = self.attr('label') + "EQE * phi_BB [J$^{-1}$ s$^{-1}$ m$^{-2}$]"

        1/16 * 8 * pi**3 = 1/2 pi**3
        2 pi
        """
        ...
        return ERE, Curve(
            [nm[mask], integrand[mask] / (CST_q) ** 2],
    ...
    def ERE_GUI(self, Voc, T, Emin="auto", EminUnit="nm"):
        ...
        print(
            "External radiative efficiency estimate:",
            "{:.2E}".format(ERE),
            "(input Voc:",
            Voc,
            ")",
    ...
    def _materialRefractivek(self, material):
        ...
        try:
            ...
            print(
                "CurveEQE._materialRefractivek: please choose a material, ",
                "or a file with (nm,k) data as 2-column. Input:",
                material,
            )
    ...
    def func_absorptionedge(self, nm, thickness, I0, material="CdS"):
        try:  # check parameters are numeric
            ...
            print(
                "CurveEQE.func_absorptionedge: Did you really enter",
