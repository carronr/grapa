...
class GraphScaps(Graph):
    ...
    def isFileReadable(
        ...
        if fileext in [".iv", ".cv", ".cf", ".qe"]:
            if line1.startswith("SCAPS ") and "ELIS-UGent: Version scaps" in line1:
                return True
            if line1 == "SCAPS output generated by a script":
                return True
        return False
...
class Simulations:
    ...
    def parse_data(self, file, lineheader):
        ...
        _line = file.readline()  # expect 1 empty line
        while True:
            line = file.readline()
            if len(line) <= 1:
                break
