...
from typing import Union, List, Any, Optional, Type, TYPE_CHECKING
from enum import Enum

from grapa.curve import Curve
from grapa.shared.constants_internals import GRAPHTYPE_UNDETERMINED
from grapa.shared.keywords_loader import keywords_graph
from grapa.shared.error_management import FileNotReadError, issue_warning
from grapa.shared.maths import is_number
from grapa.parse.parser_generic import FileParserGeneric, _identify_delimiter
from grapa.parse.parser_database import FileParserDatabase
...
class ReadAs(Enum):
    """for complement={"readas": "database"},
    tested: 'readas' in ['database', 'generic']"""
...
def load_graph(
    ...
    graph_factory: Optional[Type["Graph"]] = None,  # scheme to avoid circular imports
):
    """Loads the content of a Graph, called by Graph constructor"""
    # if str
    if isinstance(filename, str):
        ...
        # content of the file - complement['isfilecontent'] must be true
        graph.filename = filename
        FileParserDispatcher.read(graph, complement=complement)
        return
    ...
    if (
        ...
        FileParserDispatcher.append_curve_from_datainput(
            graph, filename, attributes=complement
        )
        return

    # else: a list was provided - open first file, then merge the others one by one
    ...
    graph.filename = filename[0]
    FileParserDispatcher.read(graph, cpl[0])
    if len(filename) > 1:
        if graph_factory is None:
            graph_factory = type(graph)
        for i in range(1, len(filename)):
            gr = graph_factory(config=graph.configfile)
            if len(complement) > 1:
                load_graph(gr, filename[i], complement=cpl[i])
            else:
                load_graph(gr, filename[i])
            graph.merge(gr)
...
def file_read_first3lines(filename: str) -> tuple:
    ...
    try:
        ...
        issue_warning(logger, msg, filename)
    except PermissionError:
        msg = "Permission error: %s"
        issue_warning(logger, msg, filename)
...
class FileParserDispatcher:
    ...
    @classmethod
    def read(cls, graph: "Graph", complement: Union[dict, str] = ""):
        """
        ...
        for key in complement:
            try:
                if key.lower() in keywords_graph_keys:
        ...
        readas = attributes["readas"] if "readas" in attributes else ""
        if isinstance(readas, ReadAs):
            readas = readas.value
            attributes["readas"] = readas
    ...
    @classmethod
    def _which_default_parse_method(cls, content, attributes=None):
    ...
    def readDataFromFileTxt(
        ...
        if not isfilecontent:
            try:
                ...
                msg = "%s for file: %s"
                raise FileNotReadError(msg % (type(e), e)) from e
            except (IOError, OSError, UnicodeError) as e:
                msg = "Exception readDataFromFileTxt. %s, %s."
                msa = (type(e), e)
                logger.error(msg, *msa, exc_info=True)
                raise FileNotReadError(msg % msa) from e
        else:
        ...
        func = cls._which_default_parse_method(content, attributes)
        if not isfilecontent:
    ...
    def readDataFromFileGeneric(
        ...
        """
        graph.update({"meastype": GRAPHTYPE_UNDETERMINED})
    ...
    @classmethod
    def _funcReadDataFile(cls, content, attributes=None):  # @pylint: disable=C0103
        """Deprecated, use _which_default_parse_method instead"""
        issue_warning(logger, "Deprecated", category=DeprecationWarning)
        return cls._which_default_parse_method(content, attributes=attributes)
