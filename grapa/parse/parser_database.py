...
class FileParserDatabase:
    ...
    def _what_is_to_update(content: List[list], graph: "Graph"):
        ...
        for i, line in enumerate(content):
            ...
            if not only_text:
                # if not (len(line) == 2 and maxlencols2 > 2):
                if maxlencols > 2 and len(line) > 2:
                    break  # several values on the line -> presumed a column keyword-val
                if maxlencols2 == 2:
                    if not (len(line) == 2 and n_val == 1):
                        break  # 2-column file: text\tnumeric still part of to_update
    ...
    def _populate_data(graph: "Graph", content: List[List], nlines_headers: int):
        ...
        for i, line in enumerate(content[nlines_headers:]):
            ...
            for j in range(len(line) - 1):
                graph[j].set_x(rowlabelnums[-1], index=i)
                if is_number(line[j + 1]):
                    graph[j].set_y(float(line[j + 1]), index=i)
