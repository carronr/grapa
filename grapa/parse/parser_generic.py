...
def _identify_delimiter(lines: list, print_if_issue=True) -> str:
...
class FileParserGeneric:
    ...
    def read(
        ...
        **kwargs
        ...
        if singlecurve:
            ...
            data, cols_nan, collabels = cls._add_arange_1col(data, cols_nan, collabels)
    ...
    def _get_delimiters(lines, kwargs, is_source_file):
        ...
        if "delimiter" in kwargs and isinstance(kwargs["delimiter"], str):
            ...
            delimiter = _identify_delimiter(lines, is_source_file)
    ...
    @staticmethod
    def _add_arange_1col(data, cols_nan, collabels):
        if len(data.shape) == 2 and data.shape[0] == 1 and data.shape[1] > 0:
            tmp = np.empty((2, data.shape[1]))
            tmp[1, :] = data[0, :]
            tmp[0, :] = np.arange(data.shape[1])
            data = tmp
            cols_nan.insert(0, False)
            collabels.insert(0, "index")
            print("Parse data: 1-D data detected, added arange as x axis.")
        return data, cols_nan, collabels
    ...
    @classmethod
    def _create_curves(cls, graph, data, attributes, cols_nan: list, collabels):
        coly_idx = []
        # parse through data
        col_x = 0
