...
class MetadataContainer:
    ...
    Note: uses the grapa recorder to undo/redo actions.
    Suggestion to grapa users: it may be advantageous to stick to the methods defined in
    classes Curve and Graph, which behave consistently:
    attr(key), update({key: value}), has_attr(key), get_attributes(), attr_pop(key))
    It avoids the technicality that Graph constains 2 MetadataContainer objects.
    """
