...
# TODO: Handle subplotskwargs: sharex True, 'col'
    ...
    def plot(self, filesave='', imgFormat='', figsize=(0, 0), ifSave=True, ifExport=True, figAx=None, ifSubPlot=False, handles=None):
        ...
        # generates boxplot and violinplot, differently than others
        if len(boxplot['y']) > 0:
            bxpltkwargs = {}
            # brave version - 2024.02
            for key in boxplot:
                if key not in ["i", "y", "color"]: # y: as arg, color handled below. i: grapa internal workings
                    bxpltkwargs.update({key: boxplot[key]})
            boxplot_dict = ax.boxplot(boxplot['y'], **bxpltkwargs)
            # cautious version - until 2024.02
