...
from grapa.gui.GUImisc import imageToClipboard, TextWriteable
...
#        tk.Label(frame, text='\n').pack(side='top')
    ...
    def enableCanvasCallbacks(self):
        ...
        def callback_notifyCanvas(event):
            ...
            if (event.xdata is not None and self.varDataPickerRestrict.get()
                ...
                curve = self.back_graph[self.varDataPickerCurve.get()]
                if curve is not None:
                    xdata, ydata, idx = curve.getPointClosestToXY(xdata, ydata, alter=self.back_graph.attr('alter'))
    ...
    def callback_dataPickerCurve(self, *args):
        ...
        self.observableDataPickerCurve.update_observers(self.back_graph[curve] if curve > -1 else None)
...
#        self.ChkBtnToClipboardAlter.pack(side='left')
    ...
    def fillUIFrameRightAction(self, frame, curve):
        ...
        if curve != -1:
            ...
            tmp = []
            funcList = self.back_graph[curve].funcListGUI(graph=self.back_graph, graph_i=curve)
    ...
    def plotChangeView_identify(self):
        ...
        alter = self.back_graph.attr('alter')
        if alter == '':
        ...
        typePlot = self.back_graph.attr('typeplot')
    ...
    def curveAction(self, j):
        ...
        def executeFunc(curve, func, args, kwargs):
            ...
            idx = curve + 1
            while idx < len(self.back_graph):
                type_ = self.back_graph[idx].attr('type')
            ...
            if isinstance(res, Curve):
                ...
                if self.varPrintCommands.get():
                    print("graph.append(res" + ('' if idx == len(self.back_graph) else ', idx=' + str(idx)) + ")")
        ...
        if len(curves) > 1:
            funcListRef = self.back_graph[curve].funcListGUI(graph=self.back_graph, graph_i=curve)
            for c in curves:
                if c != curve:
                    funcListOth = self.back_graph[c].funcListGUI(graph=self.back_graph, graph_i=c)
        ...
        for c in keys:
            if len(keys) > 1:
                lbl = self.back_graph[c].attr('label', '')
    ...
    def castCurve(self):
        ...
        for curve in curves:
            if curve > -1 and curve < len(self.back_graph):
    ...
    def curveShift(self, upDown, relative=True):
        ...
        for curve in curves:
            ...
            if curve == 0:
                idx2 = max(idx2, 0)
            if curve == len(self.back_graph)-1 and relative:
                idx2 = min(idx2, 0)
            if relative:
                ...
                if idx2 < curve or (idx2 == curve and curve == 0):
                    upDown += 1
                elif idx2 > curve or (idx2 == curve and curve >= len(self.back_graph)-1):
                    upDown -= 1
            # self.back_graph.swapCurves(curve, upDown, relative=True)
        for i in range(len(selected)):
            selected[i] = max(0, min(len(self.back_graph)-1, selected[i]))
    ...
    def curveShiftBott(self):
        self.curveShift(len(self.back_graph)-1, relative=False)
    ...
    def curveShowHideAll(self):
        if len(self.back_graph) > 0:
            new = '' if self.back_graph[0].isHidden() else 'none'
            for curve in self.back_graph:
            ...
            if curve >= 0:
                ...
                if self.varDataPickerSaveTransform.get():
                    ...
                    if isinstance(alter, str):
                        alter = ['', alter]
                    x = self.back_graph[curve].x_offsets(index=idx, alter=alter[0])[0]
                    y = self.back_graph[curve].y_offsets(index=idx, alter=alter[1])[0]
                ...
                if self.varDataPickerSaveCurveSpec.get():
                    ...
                    x, y, attrUpd = self.back_graph[curve].getDataCustomPickerXY(idx, alter=alter)
        return x, y, attrUpd
    ...
    def popupAnnotations(self):
        ...
        self.test = GuiManagerAnnotations(self.windowAnnotate, self.back_graph,
    ...
    def popupDataEditor(self):
        ...
        self.test = GuiDataEditor(self.windowDataEditor, self.back_graph,
    ...
    def updateUI_graph(self):
        ...
        # quick modifs
        xlim = self.back_graph.attr('xlim', ['', ''])
        ylim = self.back_graph.attr('ylim', ['', ''])
        ...
        self.varQuickModsXlabel.set(str(self.back_graph.attr('xlabel')).replace('\n', '\\n'))
        self.varQuickModsYlabel.set(str(self.back_graph.attr('ylabel')).replace('\n', '\\n'))
    ...
    def callback_updateCrosshair(self, draw=True):
        ...
        if self.varDataPickerCrosshair.get():
            ...
            curve = self.varDataPickerCurve.get()
            alter = self.back_graph.attr('alter')
            if curve >= 0 and curve >= len(self.back_graph):
                curve = len(self.back_graph) - 1
            ...
            if restrict and curve >= 0 and not np.isnan(idx):
                posx = self.back_graph[curve].x_offsets(index=idx, alter=alter[0])
                posy = self.back_graph[curve].y_offsets(index=idx, alter=alter[1])
    ...
    def treeSelectItem(self, a):
        ...
        self.observableCurve.update_observers(self.back_graph[curve] if curve > -1 and curve < len(self.back_graph) else curve)
        ...
        if curve < 0:
            ...
            self.castCurveList = self.back_graph[curve].castCurveListGUI(onlyDifferent=False)
            ...
            self.varCastCurve.set(self.back_graph[curve].classNameGUI())
    ...
    def newPropertySelect(self, *args):
        ...
        if self.varNewPropVal.get() == '' or (self.varNewPropVal.get() == self.varNewPropValPrevious):
            existingVal = self.back_graph.attr(prop) if curve == -1 else self.back_graph[curve].attr(prop)
    ...
    def updateProperty(self, curve, key, val, ifUpdate=True, varType='auto'):
        ...
        for c_ in curve:
            ...
            if c < 0:
                ...
                self.back_graph[c].update({key: val})
                if self.varPrintCommands.get():
                    valstr = str(val) if not isinstance(val, str) else "'"+val+"'"
    ...
    def executeGraphCurveMethod(self, curve, method, *args, **kwargs):
        out = getattr(self.back_graph[curve], method)(*args, **kwargs)
        if self.varPrintCommands.get():
    ...
    def curveDeleteAllHidden(self):
        ...
        toDel = []
        for c in range(len(self.back_graph)):
            if self.back_graph[c].isHidden():
                toDel.append(c)
    ...
    def curveDataToClipboard(self):
        ...
        if not ifAttrs:
            labels = [self.back_graph[c].attr('label').replace('\n', '\\n') for c in curves]
            content += ('\t' + '\t\t\t'.join(labels) + '\n')
        else:
            keys = []
            for c in curves:
                for key in self.back_graph[c].getAttributes():
                    if key not in keys:
                        keys.append(key)
            keys.sort()
            for key in keys:
                labels = [str(self.back_graph[c].attr(key)).replace('\n', '\\n') for c in curves]
    ...
    def clipboardToGraphOpen(self):
        ...
        print('Import data from clipboard ('+str(len(self.back_graph))
              + ' Curves found).')
        self.updateUI()
    ...
    def setAutoScreenDPI(self):
        ...
        figsize = self.back_graph.attr('figsize', Graph.FIGSIZE_DEFAULT)
    ...
    def saveData(self):
        if self.back_graph.attr('meastype') not in ['', FILEIO_GRAPHTYPE_GRAPH]:
    ...
    def saveDataAs(self, filesave=''):
        ...
        if self.back_graph.attr('meastype') == '':
    ...
    def folder_JV_combined(self):
        ...
        if self.back_folder != '':
            ...
            # for y axis adapted to data
            self.back_graph = processJVfolder(self.back_folder, ylim=(-np.inf, np.inf), groupCell=True, fitDiodeWeight=fitDiodeWeight, newGraphKwargs=self.newGraphKwargs)
        self.updateUI()
    ...
    def folder_JV_all(self):
        ...
        if self.back_folder != '':
            ...
            fitDiodeWeight = stringToVariable(self.FolderFitDiodeWeight.get())
            # for y axis adapted to data
            self.back_graph = processJVfolder(self.back_folder, ylim=(-np.inf, np.inf), groupCell=False, fitDiodeWeight=fitDiodeWeight, newGraphKwargs=self.newGraphKwargs)
        self.updateUI()
    ...
    def file_JscVoc(self):
        ...
        self.back_graph = script_processJscVoc(file, newGraphKwargs=self.newGraphKwargs, ROIJsclim=ROIJsclim)
        self.updateUI()
