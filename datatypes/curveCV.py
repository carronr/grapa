...
class CurveCV(Curve):
    ...
    def alterListGUI(self):
        ...
        out.append(['Carrier density N_CV [cm-3] vs V',
                    ['', 'CurveCV.y_CV_Napparent'], ''])
        out.append(['Carrier density N_CV [cm-3] vs depth [nm]',
    ...
    def CurveCV_fitVbiN(self, Vlim=None, silent=False):
        ...
        attr = {'color': 'k', 'area': self.getArea(), '_popt': [Vbi, N_CV],
                '_fitFunc': 'func_MottSchottky', '_Vlim': Vlim,
                'filename': 'fit to '+fname}
        if not silent:
            print('Fit Mott-Schottky plot: Vbi =', Vbi, 'V, apparent doping',
                  'N_CV =', '{:1.4e}'.format(N_CV) + '.')
        return CurveCV([self.x(), self.func_MottSchottky(self.x(), Vbi, N_CV)],
                       attr)
    ...
    def CurveCV_0V(self, Vtarget=0):
        ...
        if i > 0 and i < len(self.x()) - 1:
            ...
            curve.update({'linespec': 's', 'markeredgewidth': 0,
                          'labelhide': 1,
    ...
    def printHelp(self):
        ...
        print(' - Mott-Schottky: [1/(C cm-2)^2] vs V. Allows extraction of',
              'built-in voltage V_bi (intercept with y=0) and carrier density',
              '(from slope).')
        print(' - Carrier density N_CV [cm-3] vs V: the apparent carrier',
              'density is calculated from formula:')
        print('   N_CV = -2 / (eps0 epsr d/dV(C^-2)).')
        print(' - Carrier density N_CV [cm-3] vs depth [nm]: the apparent',
              'depth is calculated from parallel plate capacitor:')
        print('   C = eps0 epsr / d.')
        print('Analysis functions:')
        print(' - Set area: can normalize input data. For proper data analysis',
              'the units should be [nF cm-2].')
        print(' - Extract N_CV, Vbi: fits the linear segment on the',
              'Mott-Schottky plot.')
        print('   Select the suitable ROI before fitting (enter min and max',
              'Voltages).')
        return True
